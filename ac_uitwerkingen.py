...
def matrix_vector():
    ...
    RNG().set(1).consume_entropy(0x06, -0x14, 0x14)
...
def determinanten():
    ...
    M = random_tensor(r"\textbf{M}", (2,2), singular=matrix_nd, ret=True, details=False)
    N = random_tensor(r"\textbf{N}", (2,2), singular=matrix_gd, ret=True, details=False)
    O = random_tensor(r"\textbf{O}", (2,2), singular=matrix_nd, ret=True, details=False)
    P = random_tensor(r"\textbf{P}", (3,3), singular=matrix_nd, interval=(0,5), ret=True, details=False)
    Q = random_tensor(r"\textbf{Q}", (3,3), singular=matrix_gd, interval=(0,5), ret=True, details=False)
...
def rank():
    ...
    M = random_tensor(r"\textbf{M}", (2,2), singular=matrix_nd, interval=(0,5), ret=True, details=False)
    stats(M, 'M')
    display(Markdown("<hr>"))

    N = random_tensor(r"\textbf{N}", (2,2), singular=matrix_gd, interval=(0,5), ret=True, details=False)
    stats(N, 'N')
    display(Markdown("<hr>"))
    
    O = random_tensor(r"\textbf{O}", (3,3), singular=matrix_nd, interval=(0,5), ret=True, details=False)
    stats(O, 'O')
    display(Markdown("<hr>"))
    
    P = random_tensor(r"\textbf{P}", (3,3), singular=matrix_gd, interval=(0,5), ret=True, details=False)
    stats(P, 'P')
    display(Markdown("<hr>"))
    
    Q = random_tensor(r"\textbf{Q}", (2,3), singular=matrix_ns, interval=(0,5), ret=True, details=False)
    stats(Q, 'Q')
    display(Markdown("<hr>"))
    
    R = random_tensor(r"\textbf{R}", (2,3), singular=matrix_ns, interval=(0,5), ret=True, details=False)
    stats(R, 'R')
...
def derivatives():
    ...
    display(Markdown(f"""\\begin{{align}}f^\\prime(x) 
      &= {-b}\\cdot{c}({a-1}- {ho(b)}{xterm}) \\\\[1mm]
      &= {ho(b*b*c)}{xterm} {-b*c*(a-1)}
      \\end{{align}}"""))
    ...
    display(Markdown(f"""\\begin{{align}}g^\\prime(x) 
      &= {(a-1)*b}x^{ho(b-1)}\\ {tan} + {a-1}x^{b}{c*d}x^{ho(d-1)}\\ {sec} \\\\[1mm]
      &= {(a-1)*b}x^{ho(b-1)}\\ {tan} + {(a-1)*c*d}x^{{{ho(b+d-1)}}}\\ {sec} \\\\[1mm]
      &= {ho(g)}x^{{{ho(b-1)}}}\\ \\big({ho((a-1)*b/g)}{tan} + {ho((a-1)*c*d/g)}x^{{{d}}}\\ {sec}\\big)
      \\end{{align}}"""))
    ...
    g = np.gcd(c,b-1)
    if g != 1:
        simplification = f"\\\\[2mm] &= \\frac{{{int((b-1)/g)}-{ho(c*d/g)}x^{{{d-1}}}}}{{({ho((b-1)/g)}x-{ho(c/g)}x^{d})\\ \\text{{ln}}({a})}}"
    else:
        simplification = ""
    display(Markdown(f"""\\begin{{align}}h^\\prime(x)
     &= \\frac{{{b-1}-{ho(c*d)}x^{{{d-1}}}}}{{({b-1}x-{c}x^{d})\\ \\text{{ln}}({a})}} {simplification} \\end{{align}}"""))
    ...
    display(Markdown(f"""\\begin{{align}}\\frac{{d^2y}}{{dx^2}}
      &= \\frac{{dy}}{{dx}}x^{a} - {ho(b-1)}\\frac{{dy}}{{dx}} y \\\\[1mm]
      &= {a}x^{ho(a-1, True)} - {ho(b-1)} (x^{a} - {ho(b-1)}y) \\\\[1mm]
      &= {a}x^{ho(a-1, True)} - {ho(b-1)}x^{a} + {ho((b-1)**2)}y
      \\end{{align}}"""))
    ...
    display(Markdown(f"""\\begin{{align}}
    \\frac{{dy}}{{dx}}({ho(a-1)}x^3y - {ho(b-1)}x^2 + {ho(c-1)}y^4) &= \\frac{{dy}}{{dx}} {2*d} \\\\[2mm]
    {ho(a-1)}\\frac{{dy}}{{dx}}x^3y - {ho(b-1)} \\frac{{dy}}{{dx}} x^2 + {ho(c-1)}\\frac{{dy}}{{dx}}y^4 &= 0 \\\\[2mm]
    {ho(a-1)}(3x^2y + x^3 \\frac{{dy}}{{dx}}) - {ho(b-1)}(2x) + {ho(c-1)}(4y^3) \\frac{{dy}}{{dx}} &= 0 \\\\[2mm]
    {ho(3*(a-1))}x^2y + {ho(a-1)}x^3 \\frac{{dy}}{{dx}} - {ho(2*(b-1))}x + {ho(4*(c-1))}y^3 \\frac{{dy}}{{dx}} &= 0 \\\\[2mm]
    {ho(a-1)}x^3 \\frac{{dy}}{{dx}} + {ho(4*(c-1))}y^3 \\frac{{dy}}{{dx}} &= {ho(2*(b-1))}x - {ho(3*(a-1))}x^2y \\\\[2mm]
    ({ho(a-1)}x^3 + {ho(4*(c-1))}y^3) \\frac{{dy}}{{dx}} &= {ho(2*(b-1))}x - {ho(3*(a-1))}x^2y \\\\[2mm]
    \\frac{{dy}}{{dx}} &= \\frac{{{ho(2*(b-1))}x - {ho(3*(a-1))}x^2y}}{{{ho(a-1)}x^3 + {ho(4*(c-1))}y^3}} \\\\[2mm]
    \\end{{align}}"""))
...
def integrals_billy():
    ...
    display(Markdown("**(a)**"))
    display(Markdown(f"""\\begin{{align}}
      \\int_{{{frac(min(a,b))}\\pi}}^{{{frac(max(a,b)+2)}\\pi}} -{c} \\text{{sin}}(x)\\ dx
      &= -{c} \int_{{{frac(min(a,b))}\\pi}}^{{{frac(max(a,b)+2)}\\pi}} \\text{{sin}}(x)\\ dx \\\\[1mm]
      &= -{c}\cdot -\\text{{cos}}(x)\\ \\biggr\\rvert_{{{frac(min(a,b))}\\pi}}^{{{frac(max(a,b)+2)}\\pi}} \\\\[1mm]
      &= {c} \\text{{cos}}(x)\\ \\biggr\\rvert_{{{frac(min(a,b))}\\pi}}^{{{frac(max(a,b)+2)}\\pi}} \\\\[1mm]
      &= {c} \\left(\\text{{cos}}\\left({frac(max(a,b)+2)}\\pi\\right) - \\text{{cos}}\\left({frac(min(a,b))}\\pi\\right)\\right) \\\\[1mm]
